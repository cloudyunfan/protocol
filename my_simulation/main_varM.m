...
%         tic
        ...
        [ReTX_time_pre,Def_Time_Pre,CSMA_Sta_Pre,PL_RAP,PS_RAP,Colli_RAP,ELE_RAP,Succ_TX_time_RAP,E_buff,E_flow,Count,ELE_tx] = slotCSMACA_unsat_new000(len_RAP,CSMA_Sta_Pre,Def_Time_Pre,RAP_CHN_Sta,ReTX_time_pre,CW,last_TX_time_RAP_ini,E_buff);%ELE_RAP,（倒数第四）,E_buff,E_flow（最后两个）等一下改 CW可以全局传过去
%         toc
...
%         hist_B(j,:) = B_buff;
    ...
    for up=1:length(UPclass)
        ...
        EH_total(up,m) = mean( sum( EH_sp(:,indUP) ) );  %总采集到的能量
%         EH_of_t(up,indE) = mean( sum( EH_of_sp(:,indUP) ) );
        ...
        PS_RAP_total(up,m) = mean( sum( PS_RAP_sp(:,indUP) ) );      %总的RAP阶段发送的超帧数，取所有节点的平均数   
        PL_RAP_total(up,m) = mean( sum( PL_RAP_sp(:,indUP) ) );      %总的RAP阶段发送的超帧数，取所有节点的平均数   
        Count_total(up,m) = mean( sum( Count_sp(:,indUP) ) ); 
        
        Colli_t(up,m) = mean( sum( Colli_RAP_sp(:,indUP) ) );  %总冲突数，取所有节点的平均数                
        Interval_avg(up,m) = mean( Intv(indUP) );  %平均成功发包间隔 ,去各节点的平均数
        Ulit_rate(up,m) = mean( Slot_ulti(indUP) ); %平均信道利用率
%         Pktloss = PL_t./(PL_t+PS_t);
        Pktloss_rate(up,m) = mean( sum( PL_RAP_sp(:,indUP) )./sum( PS_RAP_sp(:,indUP) ) ) ;   %将属于同一优先级的节点的平均丢包率保存起来        
