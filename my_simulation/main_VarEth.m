...
global E_th
global N Tslot Data_rate TB Pbg Pgb CW CWmin CWmax UP UPnode Pkt_len   E_TX E_CCA Tsim P1_x %isMAP lambdaE Emax Bmax isRAP
%------------802.15.6相关的全局参数---------
UP = 0:7;  %8个优先级
...
%         hist_B(j,:) = B_buff;
    ...
    for up=1:length(UPclass)
        ...
        EH_total(up,alpha) = mean( sum( EH_sp(:,indUP) ) );  %总采集到的能量
%         EH_of_t(up,indE) = mean( sum( EH_of_sp(:,indUP) ) );
        
        ELE_RAP_t(up,alpha) = mean( sum( ELE_RAP_sp(:,indUP) ) );
        ELE_RAP_tx_total(up,alpha) = mean( sum( ELE_RAP_tx(:,indUP) ) );
        PS_RAP_total(up,alpha) = mean( sum( PS_RAP_sp(:,indUP) ) );      %总的RAP阶段发送的超帧数，取所有节点的平均数   
        PL_RAP_total(up,alpha) = mean( sum( PL_RAP_sp(:,indUP) ) );      %总的RAP阶段发送的超帧数，取所有节点的平均数   
        Count_total(up,alpha) = mean( sum( Count_sp(:,indUP) ) ); 
        
        Colli_t(up,alpha) = mean( sum( Colli_RAP_sp(:,indUP) ) );  %总冲突数，取所有节点的平均数                
        Interval_avg(up,alpha) = mean( Intv(indUP) );  %平均成功发包间隔 ,去各节点的平均数
        Ulit_rate(up,alpha) = mean( Slot_ulti(indUP) ); %平均信道利用率
%         Pktloss = PL_t./(PL_t+PS_t);
        Pktloss_rate(up,alpha) = mean( sum( PL_RAP_sp(:,indUP) )./sum( PS_RAP_sp(:,indUP) ) ) ;   %将属于同一优先级的节点的平均丢包率保存起来        
                                                           
    end
%       %-----------------统计整个WBAN的结果---------------------------   

%         Act_time(indE) = mean(actTime_sp);
        EH_WBAN(alpha) = sum( sum(EH_sp) )/N;
        %yf
        ELE_of_WBAN(alpha) = sum( sum(ELE_RAP_sp) )/N;
        Interval_WBAN(alpha) = mean(Intv);
        %yf
        ELE_WBAN(alpha) = sum( sum(ELE_RAP_sp) )/N;
        %yf
        PS_WBAN(alpha) = sum( sum(PS_RAP_sp) )/N;       
        Colli_WBAN(alpha) = sum( sum(Colli_RAP_sp) )/N;
        Pktloss_WBAN(alpha) = sum(sum( PL_RAP_sp ))/sum(sum( PS_RAP_sp ));
        
      disp(['alpha NumUP: ',num2str([alpha N])]) 
end
disp('unsaturation VaringEth simulation done!')
save('VarEth_MAC(UP0-2-4-6)(P1_x0.6)(N12).mat');