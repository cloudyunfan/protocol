...
% load('VarN_MAC(UP0-2-4-6,NH1-1-8)(P1_x0.9)(N4-4-32)EkeyBC0addCnt.mat');
load('VarEth_MAC(UP0-2-4-6)(P1_x0.6)(N12).mat');
...
% % 
figure(2);
% semilogy(eth,tau_Real);
semilogy(eth,tau_Real(1,:),'^b','markerfacecolor','b');
hold on;
semilogy(eth,tau_Real(2,:),'or','markerfacecolor','r');
hold on;
semilogy(eth,tau_Real(3,:),'dg','markerfacecolor','g');
hold on;
semilogy(eth,tau_Real(4,:),'vk','markerfacecolor','k');
hold on;
semilogy(eth,tau_DTMC(1,:),'-b',eth,tau_DTMC(2,:),'-r',eth,tau_DTMC(3,:),'-g',eth,tau_DTMC(4,:),'-k');
...
xlabel('N(energy units needed in transmission or CCA)');
ylabel('Tau(k)');
