function [ReTX_time,backoff_after,CSMA_sta,pl_t,ps_t,PL_colli,ELE_ex,TX_time,E_buff,E_flow,Count] = slotCSMACA_unsat_new0(rap_length,CSMA_sta,def_time_pre,last_CHN_sta,ReTX_times_pre,CW,last_TX_time,E_buff) %act,,B_buff
...
Count = zeros(1,N); %times of nodes transmission
%yf能量初始化
...
%             disp(['node ',num2str(ind_TX),' send Pkt ',num2str(PS_cap),' successfully!']);
            ...
            %%--------------------修改仿真变量------------------------
            CW( ind_TX(1) ) = CWmin( find(UP==UPnode( ind_TX(1) )) );  %发送成功，重置竞争窗口
            %如果发送成功则在当前时隙做标记
            if(PS_cap>0)                                
