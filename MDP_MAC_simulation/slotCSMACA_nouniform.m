function [ReTX_time,backoff_after,CSMA_sta,tdma_CHN_sta,pl_t,ps_t,PL_colli,ELE_ex,TX_time,E_overflow_t,E_EH,E_buff] = slotCSMACA_unsat(rap_length,CSMA_sta,def_time_pre,last_CHN_sta,ReTX_time_pre,CW,last_TX_time,lambdaE,E_buff,Colli_eff)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     9.lambdaE: energy harvesting speed
%     10:E_buff:能量缓存区
%     11.Colli_eff: 冲突影响因子，影响窗口更新过程
% Output:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
global Data_rate Pkt_len Tslot Pbg Pgb CWmin CWmax UP UPnode E_TX E_CCA Emax 
...
%                     Backoff_time(n) = randint(1,1,[1,CW(n)]);  %均匀分布生成退避计数器
                    Backoff_time(n) = gen_Backoff(CW(n),E_buff(n),Emax);  %使用分段均匀分布来生成退避计数器
            ...
            if( CSMA_sta(n)==1&&backoff_lock(n)==0 ) 
                if(Backoff_time(n) == 0) %退避到0，进入发送状态
                      ...
                      if( CHNflag(n)==0&&E_buff(n)>=E_TX ) % channel is idle,energy is enough,there has pkt to send                                
...
%                     CW(n1) = min(2*CW(n1),CWmax(find(UP==UPnode(n1))));%窗口加倍
                    %使用窗口更新因子更新窗口
                    CW(n1) = min( round(CW(n1)*(1+Colli_eff(n1))),CWmax(find(UP==UPnode(n1))) );
                end                                    
            end
        end               
    else
...
%                 TX_time{ind_TX} = [TX_time{ind_TX},last_TX_time(ind_TX)];
    ...
    %更新energy buffer
    [e_overflow,e,E_buff] = E_update(1,E_buff,N,Emax,lambdaE);
    E_overflow_t = E_overflow_t + e_overflow;  %累计能量溢出 
    E_EH = E_EH + e;  %累计能量采集
    t = t + 1;
end %end while
TX_time = cell(1,N);
