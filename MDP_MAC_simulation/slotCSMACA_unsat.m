function [ReTX_time,backoff_after,CSMA_sta,tdma_CHN_sta,pl_t,ps_t,PL_colli,ELE_ex,TX_time,E_buff,B_buff] = slotCSMACA_unsat(act,rap_length,CSMA_sta,def_time_pre,last_CHN_sta,ReTX_time_pre,CW,last_TX_time,E_buff,B_buff)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
global Data_rate Pkt_len Tslot Pbg Pgb CWmin CWmax UP UPnode E_TX E_CCA isRAP isMAP
...
%                 TX_time{ind_TX} = [TX_time{ind_TX},last_TX_time(ind_TX)];
            ...
            if(act(ind_TX)==3||act(ind_TX)==4)
                if(act(ind_TX)==3)
                   isRAP(ind_TX) = 0;  %如果行为3，则成功一次发送数据包后停止使用RAP 
                end
               isMAP(ind_TX) = 1;  %节点可以使用MAP阶段了
            else
               if(act(ind_TX)==1)
                   isRAP(ind_TX) = 0;  %如果行为1，则成功一次发送数据包后停止使用RAP 
                end
               isMAP(ind_TX) = 0;  %成功释放MAP阶段使用权
            end    
            if(act(ind_TX)==2||act(ind_TX)==4)%只有传输的是正常数据，才统计发包数和丢包数
