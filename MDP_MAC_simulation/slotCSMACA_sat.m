function [ReTX_time,backoff_after,CSMA_sta,tdma_CHN_sta,pl_t,ps_t,PL_colli,ELE_ex,TX_time] = slotCSMACA_sat(rap_length,CSMA_sta,def_time_pre,last_CHN_sta,ReTX_time_pre,CW,last_TX_time)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
global Data_rate Pkt_len Tslot Pbg Pgb CWmin CWmax UP UPnode E_TX E_CCA 

%-----------参数---------------------------------------------------------
SIFS = 0;      %最短帧间间隔
T_pkt = ceil( Pkt_len/(Data_rate*Tslot) );  %time to send a packet and receive ACK,unit slot
M = 4;        %最大重传次数
N = length(CW);  %获取使用CSMA/CA的节点数
rap_length = rap_length*T_pkt; %时间量化以1/T_pkt量化；
